...
class FinanceState:
    ...
    # Personalised AI tip card (right-hand panel). Generated by
    # ``pipeline.generate_tip(...)`` after any analysis completes; the
    # card stays empty until the user has at least one structured
    # output that the tip can riff on.
    tip: Optional[dict] = None
    tip_running: bool = False

    # Per-section demo flag (per ``ai-section.mdc``). When ``True`` the
    # pipeline returns curated mock data without spending tokens so the
    # user can showcase the section. Toggle lives in the section
    # header menu.
