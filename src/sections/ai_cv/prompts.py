...
MATCH_ANALYSIS_SYSTEM = (
    ...
    "  can prepare for the hard ones. For EACH question return:\n"
    "    - question: the wording the interviewer would use.\n"
    "    - why_asked: 1-2 sentences explaining what the interviewer is\n"
    "      trying to learn (tied to a specific job-spec requirement or\n"
    "      identified gap, NOT generic best-practice).\n"
    "    - suggested_answer: a 60-120 word draft answer in STAR shape\n"
    "      (Situation, Task, Action, Result) using EVIDENCE FROM THE\n"
    "      CANDIDATE JSON. Never invent metrics or jobs the candidate\n"
    "      did not list. If a gap question has no candidate evidence,\n"
    "      coach the candidate on how to answer honestly while pivoting\n"
    "      to relevant adjacent experience.\n"
    "* skill_gap_plan: prioritised list of objects. For EACH gap return:\n"
    "    - skill: the skill / tool / behaviour the candidate needs.\n"
    "    - action: one short imperative line summarising what to do.\n"
    "    - timeline_weeks: realistic 1-52.\n"
    "    - criticality: one of 'critical' (JD explicitly requires it),\n"
    "      'important' (strongly preferred), 'nice_to_have'\n"
    "      (differentiator).\n"
    "    - why_it_matters: 1-2 sentences explaining the role-specific\n"
    "      consequence of leaving this gap unaddressed - cite the JD\n"
    "      where possible.\n"
    "    - learning_path: 2-6 ORDERED concrete steps (course name,\n"
    "      book, repo, exercise). One short imperative line each. Be\n"
    "      specific - 'finish the official Storybook 8 Intro tutorial'\n"
    "      not 'study Storybook'.\n"
    "    - suggested_project: a small, portfolio-able project the\n"
    "      candidate can build to PROVE the skill. 1-2 sentences\n"
    "      describing scope and visible outcome (repo / blog post /\n"
    "      Figma file)."
)
...
INTERVIEW_PREP_SYSTEM = (
    ...
    "MatchAnalysis.interview_questions is a list of objects with\n"
    "{question, why_asked, suggested_answer}. Use them all - do NOT\n"
    "drop any. Add 2-3 classic competency questions tied to the role\n"
    "if the list is shorter than 7.\n\n"
    "Sections:\n\n"
    "## Likely interview questions\n"
    "- For each item, format as:\n"
    "  ### {n}. {question}\n"
    "  **Why it gets asked:** {why_asked}\n"
    "  **Suggested answer:** {suggested_answer rephrased into 2-4\n"
    "  natural-sounding bullets using STAR shape and ONLY candidate\n"
    "  evidence. If suggested_answer is missing for a gap question,\n"
    "  draft an honest answer that acknowledges the gap and pivots to\n"
    "  the closest adjacent experience.}\n\n"
...
SKILL_GAP_SYSTEM = (
    HR_EXPERT_RULES
    + "\n\nTASK: Produce a SKILL-GAP CLOSING PLAN in Markdown.\n"
    "MatchAnalysis.skill_gap_plan is a list of objects with\n"
    "{skill, action, timeline_weeks, criticality, why_it_matters,\n"
    "learning_path[*], suggested_project}. Use ALL fields - do not\n"
    "drop any.\n\n"
    "Sort entries by criticality (critical first, then important, then\n"
    "nice_to_have). Format each entry as:\n\n"
    "### {Skill} - {criticality_label}\n"
    "- **Action:** {action}\n"
    ...
    "    2. {learning_path[1]}\n"
    "    ... (preserve ordering)\n"
    "- **Portfolio project:** {suggested_project}\n\n"
    "criticality_label is the human-readable form of criticality:\n"
    "'Critical', 'Important', 'Nice to have' (or the Czech equivalents\n"
    "'Kritické', 'Důležité', 'Hezké mít' when output_lang='cs').\n"
)
