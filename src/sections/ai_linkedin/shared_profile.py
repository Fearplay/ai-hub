"""Bridge between the shared career profile and AI LinkedIn's inputs.

Maps the once-uploaded CV / LinkedIn export / GitHub / notes from
:mod:`src.services.career_profile_store` (a shared service) into AI
LinkedIn's ``STATE`` so the user does not re-upload them. Opt-in via the
setup-tab banner; the upload zones stay editable for a per-run override.
"""
...
from src.sections.ai_linkedin.state import STATE, UploadedFile
...
def has_shared() -> bool:
    try:
        ...
        logger_service.log_exception("ai_linkedin.shared_profile", "has_shared_failed", exc)
        return False
...
def apply() -> None:
    """Map the shared profile's raw sources into AI LinkedIn's STATE."""
    try:
        ...
        if linkedin:
            STATE.linkedin_export = _to_uploaded(linkedin)
        github = career_profile_store.github_url()
        if github:
            STATE.github_url = github
            STATE.github_skip = False
        notes = career_profile_store.notes()
        if notes and not STATE.notes.strip():
            STATE.notes = notes
        logger_service.log_event(
            "INFO", "ai_linkedin.shared_profile", "apply_done",
            has_resume=bool(resume), has_linkedin=bool(linkedin),
            has_github=bool(github), has_notes=bool(notes),
        )
    except Exception as exc:
        logger_service.log_exception("ai_linkedin.shared_profile", "apply_failed", exc)
